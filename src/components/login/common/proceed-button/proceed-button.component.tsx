...
import "./proceed-button.css";

interface IProps {
  submitVotes: any,
  candidates: IElection.Candidate[],
  resetVotes: any,
  state: { value: number, set: any },
  isSelected: boolean
}

const ProceedButton = ({ submitVotes, candidates, resetVotes, state, isSelected }: IProps) => {
  const states = [`activate`, `loading`, `done`];

  const getCurrentState = () => {
    let currentState = ``;
    for (let i = 0; i <= state.value; i++) currentState += states[i] + ` `;
    return currentState;
  };
  return (
    <div className="proceed-button">
      <a
        onClick={() => {
          if (state.value == 0 && isSelected) {
            state.set(1);
          }
        }}
        className={`${getCurrentState()} ${isSelected ? `selected` : ``}`}
      >
        <span>
          <svg>
            <use href="#circle"></use>
        ...
        <div className="confirmation">
          ...
          <div className="actions">
            <button onClick={() => { submitVotes() }} className="btn submit">Submit</button>
